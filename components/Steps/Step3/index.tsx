...
function Step3() {
  ...
  const certificatesErrors = formik.errors.certificates as
    | FormikErrors<ICertificates>[]
    | undefined;
  const certificatesTouched = formik.touched.certificates as
    | Array<FormikTouched<ICertificates>>
    | undefined;
    ...
    if (
      ...
      setFileError((prev) => {
        ...
        formik.values.certificates.map((certificateItem: ICertificates) => {
          const alreadyExists = updatedErrors.find(
            (errorItem: IError) => errorItem.id === certificateItem.id
          );
  ...
  return (
    <>
          ...
          >
            ...
            >
              <FormControl className="portfolioLabel">
                Başlık <span className="labelRequired">*</span>
              </FormControl>
              <TextField
            ...
            >
              <FormControl className="portfolioLabel">
                Belgenin Alındığı Tarih <span className="labelRequired">*</span>
              </FormControl>
              <Grid width="60%" display="flex" flexDirection="column">
                <DatePicker
                  ...
                  format="DD/MM/YYYY"
                  value={dayjs(
                    formik.values.certificates[i].date,
                    "DD/MM/YYYY"
                  )}
                  onChange={(newValue) => {
                    formik.setFieldValue(
            ...
            <Grid size={12}>
                    ...
                    if (filesFormik.length >= 3) {
                      setFileError((prev) => {
                        const exists = prev.find(
                          (file) => file.id === certificateItem.id
                        );
                        if (exists) {
                          return prev.map((file) =>
                            file.id === certificateItem.id
                              ? { ...file, isError: true }
                              : file
                          );
                        } else {
                          return [
                            ...prev,
                            { id: certificateItem.id, isError: true },
                          ];
                        }
                      });
                    }
                    if (files) {
                      ...
                      const newFiles = await Promise.all(
                        limitedFiles.map(async (fileItem: File) => {
                          const extentedFile: IExtendFile = Object.assign(
                            fileItem,
                            {
                      ...
                      const filesFiltered =
                        calculate > 0 ? newFiles.slice(-calculate) : [];
                      ...
                      formik.setFieldValue(`certificates[${i}].files`, [
                        ...filesFormik,
                        ...filesFiltered,
                      ]);
                    }
              ...
              {Array.isArray(certificateItem?.files) &&
                ...
                certificateItem.files.map(
                  (fileItem: ICertificatesFiles, fileItemIndex) => (
                    <Grid
                      size={12}
                      key={fileItemIndex}
                      sx={{
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            const filteredFiles = currentFiles.filter(
                              (item: ICertificatesFiles) =>
                                item.id !== fileItem.id
                            );
                            formik.setFieldValue(
                              `certificates[${i}].files`,
                              filteredFiles
                            );
                          }}
                        >
              ...
              <IoMdClose
                onClick={() => {
                  const filteredCertificates =
                    formik.values.certificates.filter(
                      (item) => item.id !== certificateItem.id
                    );
      ...
      >
        ...
        >
          {" "}
          {formik.values.certificates.length === 0 ? "Yeni Ekle" : "Ekle"}{" "}
          <IoMdAdd fontSize={30} />
        </Button>
        {Boolean(formik.errors.certificates && formik.touched.certificates) &&
