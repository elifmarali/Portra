  ...
  const getFavorites = async () => {
    try {
      ...
      const myPortfolioIds: number[] = Array.isArray(myPortfolioRes.data?.data)
        ? myPortfolioRes.data.data.map((p: ICreatePortfolio) => p.id).filter((id: number) => typeof id === "number")
        : [];
      ...
      const res = await axios.get(`/api/portfolio/favoritePortfolioList`, {
        params: {
  ...
  return (
    ...
    >
      {loading ? (
        ...
        myFavoritesPortoflios.map((portfolioItem: ICreatePortfolio, portfolioItemIndex: number) => (
